...
contract FlowSender {
    ...
    function gainDaiX(uint256 amount) external {
        // Get address of fDAI by getting underlying token address from DAIx token
        TestToken fdai = TestToken(daix.getUnderlyingToken());

        // Mint 10,000 fDAI
        fdai.mint(address(this), amount);

        // Approve fDAIx contract to spend fDAI
        fdai.approve(address(daix), amount);
